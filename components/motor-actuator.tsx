...
// Removed OrbitControls, useEffect, useCallback
...
const useMotorMaterials = (isDarkMode: boolean) => {
  return useMemo(() => ({
    ...
    ledMaterial: new THREE.MeshPhongMaterial({
      ...
      emissiveIntensity: 0.8, // Initial intensity, will be animated
      shininess: 90,
      name: 'led'
    }),
...
export default function MotorActuator({ isDarkMode = false }: MotorActuatorProps) {
  ...
  // Memoize positions and calculations
  ...
  return (
    <group ref={motorGroupRef} dispose={null}>
      ...
      <mesh
        ...
        position={[-HOUSING_LENGTH / 2 - REAR_CAP_LENGTH / 2, 0, 3]} // Adjusted position based on rear cap
      >
      ...
      {/* Status LED */}
      <mesh
        // Assign the ref to the material, not the mesh
        material={materials.ledMaterial}
        ref={ledMaterialRef as any} // Assign ref to material for animation
        position={[-HOUSING_LENGTH / 2 - REAR_CAP_LENGTH / 2, CONNECTOR_BOX_SIZE.y / 2 + LED_RADIUS, 3]} // Position relative to connector box
      >
...
// Removed containerRef, sceneRef, motorRef, ledRef (mesh ref), animationFrameId
