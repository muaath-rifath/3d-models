...
export default function IoTGateway({ isDarkMode = false }: IoTGatewayProps) {
  ...
  // Refs for LED materials to animate intensity
  const powerLedMatRef = useRef<THREE.MeshStandardMaterial>(materials.powerLedMaterial);
  const internetLedMatRef = useRef<THREE.MeshStandardMaterial>(materials.internetLedMaterial);
  const wifiLedMatRef = useRef<THREE.MeshStandardMaterial>(materials.wifiLedMaterial);
  const iotLedMatRef = useRef<THREE.MeshStandardMaterial>(materials.iotLedMaterial);

  // Animation logic for LED blinking
  useFrame((state) => {
    ...
    if (powerLedMatRef.current) powerLedMatRef.current.emissiveIntensity = baseIntensity; // Solid power
    if (internetLedMatRef.current) internetLedMatRef.current.emissiveIntensity = blinkIntensity; // Blinking internet
    if (wifiLedMatRef.current) wifiLedMatRef.current.emissiveIntensity = fastBlinkIntensity; // Faster blinking WiFi
    if (iotLedMatRef.current) iotLedMatRef.current.emissiveIntensity = blinkIntensity; // Blinking IoT
  });

  // Memoize positions for vents and ports
  ...
  const ethernetPortPositions = useMemo(() => Array.from({ length: 4 }, (_, i) => [
    portStartX + portSpacing * (i + 1), // Start after power port
    BASE_SIZE.y / 2,
    -BASE_SIZE.z / 2 - ETHERNET_PORT_SIZE.z / 2
  ] as const), []);

  const usbPortPosition = useMemo(() => [
    portStartX + portSpacing * 5 + USB_PORT_SIZE.x / 2, // After ethernet ports
    BASE_SIZE.y / 2,
  ...
  return (
    <group ref={gatewayGroupRef} dispose={null}>
      ...
      {/* Power LED */}
      <mesh
        material={powerLedMatRef.current} // Use ref here
      ...
      {/* Internet LED */}
      <mesh
        material={internetLedMatRef.current} // Use ref here
      ...
      {/* WiFi LED */}
      <mesh
        material={wifiLedMatRef.current} // Use ref here
      ...
      {/* IoT LED */}
      <mesh
        material={iotLedMatRef.current} // Use ref here
